defmodule Playcode.Import.FilemakerSyncTest do
  ...
  describe "apply_plan/2" do
    test "writes the changes and logs one activity entry per play" do
      ...
      assert Playcode.Catalogue.get_play!(original.id).language == "en"

      reloaded = Playcode.Catalogue.get_play!(translation.id)
      assert reloaded.relationship_type == "traduccion"
      assert reloaded.parent_play_id == original.id

      entries = Playcode.ActivityLog.list_entries(play_id: translation.id)
    ...
    test "applying twice is a no-op the second time" do
      ...
      second = FilemakerSync.plan(index, [Playcode.Catalogue.get_play!(original.id)])
  ...
  describe "curated fields are fill-only" do
    ...
    test "force writes the conflicting value" do
      ...
      assert Playcode.Catalogue.get_play!(original.id).historical_time == "antiguedad_clasica"
    end
    ...
    test "without force the conflicting value survives" do
      ...
      assert Playcode.Catalogue.get_play!(original.id).historical_time == "edad_media"
    end
  end
  ...
  describe "composition date" do
    ...
    test "force writes the conflicting dating" do
      ...
      assert Playcode.Catalogue.get_play!(original.id).composition_date_from == 1606
    end
    ...
    test "running twice changes nothing the second time" do
      ...
      second =
        FilemakerSync.plan(index(), [Playcode.Catalogue.get_play!(original.id)], versions())
