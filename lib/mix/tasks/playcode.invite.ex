defmodule Mix.Tasks.Playcode.Invite do
  ...
  Invites a user to Playcode.
      ...
      mix playcode.invite ana@uv.es --admin --print-url
  ...
  def run(args) do
    ...
    email =
      case argv do
        [email] -> email
        _ -> Mix.raise("usage: mix playcode.invite EMAIL [--admin] [--print-url]")
      end
          ...
          case Accounts.deliver_invite(user, token, &AdminBootstrap.invite_url/1) do
            ...
            {:error, reason} ->
              ...
              The account exists. Re-run with --print-url to get the link:
                  ...
                  mix playcode.invite #{email}#{if opts[:admin], do: " --admin"} --print-url
              """)
          end
        end
